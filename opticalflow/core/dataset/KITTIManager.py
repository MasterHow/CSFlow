...
from opticalflow.utils import flow_to_image
...
class KITTIDemoManager(DatasetManagerBase):
    ...
    @classmethod
    def _postprocess_image(cls,
                           uv: np.ndarray,
                           valid_mask: np.ndarray = None) -> np.ndarray:
        """Postprocess single image.

        Input:
        uv: a 2-channel image, which indicates the optical
        flow of each pixel (u, v) in [-512, 512]
        valid_mask: a ndarray in [H,W,1] indicates the valid
        flow mask

        Output:
        a numpy.ndarray which can be passed to cv.imread
        """

        valid_mask = np.ones([uv.shape[0], uv.shape[1], 1])
        uv = uv * valid_mask

        return flow_to_image(uv, clip_flow=None)
